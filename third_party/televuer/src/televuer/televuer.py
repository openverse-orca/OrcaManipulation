...
class TeleVuer:
    def __init__(self, use_hand_tracking: bool, binocular: bool=True, img_shape: tuple=None, display_fps: float=30.0,
                       ...
                       cert_file: str=None, key_file: str=None, host: str="127.0.0.1"):
        """
        ...
        :param host: str, listen address. Defaults to loopback, reached from the headset
            through `adb reverse`. Any other address requires cert_file and key_file,
            because WebXR only starts an immersive session in a secure context.
        ...
        # Never discover credentials from environment variables, HOME, or the
        # source tree. Plain HTTP/WS is only accepted on loopback, which stays a
        # WebXR secure context and is reachable from the headset via adb reverse.
        if (cert_file is None) != (key_file is None):
            raise ValueError("[TeleVuer] cert_file and key_file must be provided together")
        if cert_file is None:
            cert_file = ""
            key_file = ""
        elif bool(cert_file) != bool(key_file):
            raise ValueError(
                "[TeleVuer] cert_file and key_file must both be non-empty or both empty"
            )
        if not cert_file and host not in ("127.0.0.1", "localhost", "::1"):
            raise ValueError(
                f"[TeleVuer] host={host!r} requires cert_file and key_file; "
