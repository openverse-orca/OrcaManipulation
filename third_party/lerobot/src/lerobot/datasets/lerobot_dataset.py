...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Check timestamps
        # Compatible with datasets 3.x (list of tensors) and 4.x (Column object)
        _ts = self.hf_dataset["timestamp"]
        _ei = self.hf_dataset["episode_index"]
        if hasattr(_ts, "__iter__") and not hasattr(_ts, "numpy"):
            timestamps = torch.stack(list(_ts)).numpy()
            episode_indices = torch.stack(list(_ei)).numpy()
        else:
            import numpy as np
            timestamps = np.array(_ts)
            episode_indices = np.array(_ei)
    ...
    def save_episode_data_only(self) -> int:
        ...
        logging.info(f"  [1/3] 等待图像写入完成（flush PNG）…")
        self._wait_image_writer()
        logging.info(f"  [2/3] 写入 parquet 数据表…")
        self._save_episode_table(episode_buffer, episode_index)
        ep_stats = compute_episode_stats(episode_buffer, self.features)
        logging.info(f"  [3/3] 更新元数据（episodes.jsonl / info.json）…")
        # Save metadata now so the next episode can start right away.
        # (Original save_episode() defers this until after encoding; here we
        # move it before encoding intentionally — the only semantic difference
        # is that meta.info["total_videos"] is incremented before the MP4 files
        # actually exist on disk, which is acceptable for live recording.)
        self.meta.save_episode(episode_index, episode_length, episode_tasks, ep_stats)

        # Timestamp sanity-check (fast, no I/O)
        ep_data_index = get_episode_data_index(self.meta.episodes, [episode_index])
        ep_data_index_np = {k: t.numpy() for k, t in ep_data_index.items()}
        check_timestamps_sync(
            episode_buffer["timestamp"],
        ...
        # Verify parquet count; skip video count assertion (videos not yet encoded)
        parquet_files = list(self.root.rglob("*.parquet"))
        assert len(parquet_files) == self.num_episodes

        # Reset buffer so add_frame() for the next episode works immediately
        self.episode_buffer = self.create_episode_buffer()
    ...
    def clear_episode_buffer(self) -> None:
        ...
        # Must flush the image writer first: PNG files may still be in-flight,
        # and shutil.rmtree will fail with ENOTEMPTY if the directory is being
        # written to concurrently.
        if self.image_writer is not None:
            self._wait_image_writer()   # block until all pending PNGs are flushed
