...
class OpenLoongInterpolator(AbstractInterpolator):
            ...
            if num_arms > 1:
                ...
                for arm_idx in range(num_arms):
                    ...
                    interpolated = self._interpolate_quaternion(
                        arm_data, use_saved_indices=True
                    )
    ...
    def _interpolate_linear(
        self, dataset: np.array, save_indices: bool = False
    ) -> np.array:
        """
        线性数据插值：每4个值插入1个新值
    ...
    def _interpolate_quaternion(
        self, dataset: np.array, use_saved_indices: bool = False
    ) -> np.array:
        """
        ...
        for i in range(num_groups):
            ...
            if (
                use_saved_indices
                and self._insertion_indices is not None
                and i < len(self._insertion_indices)
            ):
                # 使用position计算的插值位置
            ...
            noise = np.random.uniform(
                -self.noise_value, self.noise_value, size=q_interp.shape
            )
...
class OpenLoongInterpolatorAdvanced(AbstractInterpolator):
    ...
    def _cubic_spline_interpolate(self, dataset: np.array) -> np.array:
        ...
        for dim in range(dataset.shape[-1]):
            ...
            noise = np.random.uniform(
                -self.noise_value, self.noise_value, size=interpolated.shape
            )
        ...
        for i in range(n - 1):
            ...
            for j in range(1, self.interpolation_factor + 1):
                ...
                if j < self.interpolation_factor:
                    ...
                    q_interp = q_interp + noise
                    q_interp = q_interp / np.linalg.norm(
                        q_interp, axis=-1, keepdims=True
                    )
                    result.append(q_interp)
                else:
                    result.append(q2)
