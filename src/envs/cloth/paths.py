...
def resolve_cloth_level(level: str | None = None) -> str:
    """委托 ``studio_level.resolve_cloth_level_with_studio``，并自动同步本机关卡配置。"""
...
def resolve_cloth_config_path(
    ...
    *,
    ...
    2. ``cloth_sim_config.json``（``CLOTH_DEBUG=1`` 时优先 ``cloth_sim_config.debug.json``）

    ``level`` / ``agent`` 参数保留兼容，不再参与文件名选择。
    """
    del level, agent
    ...
    hit = _pick_config_candidate(CLOTH_CONFIG_BASENAME, debug=debug)
    if hit is not None:
        return hit

    raise FileNotFoundError(f"cloth config not found: {CLOTH_3D_DIR / CLOTH_CONFIG_BASENAME}")
