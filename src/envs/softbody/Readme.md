...
| `src/examples/dataCollection_cook` | **薄启动器 + 数据目录（入口）**。`RunCollection.py` 只从环境变量组装 `P23cParams` 并调用 `Start`；同时承载**单配置文件 `Config.json`**（布料 sim + 场景资产路径 + XPBD target/版本）与运行时产物（`cloth_sim_session_*.json`、`logs/`）。 |

### softbody 分层

softbody 内按职责分四层 + 一个脚本，方向是「上层调用下层、下层不感知上层」：

1. **编排器** `ProcessOrchestrator.py`（主进程）
   - 顶层入口 `Start(params)`：session 准备 → 场景/资产解析 → config 组装 → 建 env → 挂耦合 → 遥操循环。
...
3. **领域工具 / 数据契约** `domain/`
   - `paths.py`：仓库/目录路径解析与布料配置/资产路径（`PROJECT_ROOT` / `ORCA_REPO_ROOT` / `SOFTBODY_DIR` / cloth config 等）。
