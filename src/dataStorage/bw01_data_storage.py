...
class BW01DataStorage(AbstractDataStorage):
    ...
    def obs_callback(self, env: OrcaGymLocalEnv) -> dict:
        obs = {}
        joint_names = bw10_conf.l_arm["joint_names"] + bw10_conf.r_arm["joint_names"]
        joint_names = [env.joint(joint_name) for joint_name in joint_names]

        hand_names = bw10_conf.l_hand["joint_names"] + bw10_conf.r_hand["joint_names"]
        hand_names = [env.joint(hand_name) for hand_name in hand_names]

        hand_motor_names = bw10_conf.l_hand["actuator_names"] + bw10_conf.r_hand["actuator_names"]
        ...
        ee_site_names = [bw10_conf.l_arm["ee_site_name"], bw10_conf.r_arm["ee_site_name"]]
        ...
        ee_site_pos_quat = env.query_site_pos_and_quat_B(ee_site_names, [env.body(bw10_conf.base_body)])
