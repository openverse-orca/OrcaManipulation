...
def wait_ports_open(
    ...
    ready: dict = {}
    _logger.info(f"[相机] 等待推流端口就绪（最长 {timeout:.0f}s）...")
        ...
        for name in list(pending):
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                if sock.connect_ex((host, port)) == 0:
                    ready[name] = port
                    del pending[name]
                    _logger.info(f"  ✓ 端口 {port}（{name}）已就绪")
        if pending:
            _logger.info(f"  等待端口: {sorted(pending.values())}")
            time.sleep(1.0)
    if pending:
        _logger.warning(
            "[相机] 以下端口超时未就绪，将跳过：%s\n"
...
def probe_camera_hw(cameras: dict, camera_map: dict, default_hw: tuple) -> tuple:
    ...
    try:
        ...
        _logger.warning(f"相机分辨率探测失败，回退 {default_hw}: {e}")
    return default_hw


def setup_cameras(camera_map: dict) -> dict:
    """[已废弃] 启动 WebSocket 相机流，无端口就绪探测、无线程重启。

    建议改用 bring_up_cameras()：它包含 TCP 端口探测、首帧等待、线程自动重启。
    本函数仅用于向后兼容旧调用方，新代码请勿使用。
    """
    from orca_gym.sensor.rgbd_camera import CameraWrapper  # type: ignore[import]
    cameras = {}
    for name, (_key, port) in camera_map.items():
        try:
            ...
            cam.start()
            cameras[name] = cam
            _logger.info(f"相机 {name} 已启动（端口 {port}）")
        except Exception as e:
            _logger.warning(f"相机 {name} 启动失败（端口 {port}）: {e}")
    return cameras


def wait_for_cameras(cameras: dict, timeout: float = 30.0) -> None:
    """[已废弃] 等待 WebSocket 相机首帧就绪，无线程重启。
...
def extract_frames_from_mp4(
    ...
    for env_name, (key, _port) in camera_map.items():
        ...
        if not os.path.exists(mp4_path):
            _logger.warning(f"MP4 未找到，该路将输出黑帧: {mp4_path}")
            continue
        cap = cv2.VideoCapture(mp4_path)
        if not cap.isOpened():
            _logger.warning(f"MP4 无法打开，该路将输出黑帧: {mp4_path}")
            continue
...
def probe_mp4_hw(episode_video_dir: str, camera_map: dict, default_hw: tuple) -> tuple:
