...
class OpenLoongDataStorage(AbstractDataStorage):
    ...
    def obs_callback(self, env: OrcaGymLocalEnv) -> dict:
        ...
        waist_joint_name = None
        waist_angle = 0.0
        if hasattr(openloong_conf, "waist"):
            try:
                waist_joint_name = env.joint(openloong_conf.waist["joint_name"])
                waist_qpos = env.query_joint_qpos([waist_joint_name])
                waist_angle = float(waist_qpos[waist_joint_name])
                joint_names.append(waist_joint_name)
            except Exception:
                waist_joint_name = None
                waist_angle = 0.0
        ...
        try:
            ee_site_pos_quat = env.query_site_pos_and_quat_B(
                ee_site_names,
                [env.body(openloong_conf.base_body)],
        ...
        if waist_joint_name is not None:
            obs["/action/waist/angle"] = np.array([waist_angle], dtype=np.float32)
        return obs   
