...
class Gr3DataStorage(AbstractDataStorage):
    ...
    def obs_callback(self, env: OrcaGymLocalEnv) -> dict:
        obs = {}
        joint_names = gr3_conf.l_arm["joint_names"] + gr3_conf.r_arm["joint_names"]
        joint_names = [env.joint(joint_name) for joint_name in joint_names]

        hand_names = gr3_conf.l_hand["joint_names"] + gr3_conf.r_hand["joint_names"]
        hand_names = [env.joint(hand_name) for hand_name in hand_names]

        hand_motor_names = gr3_conf.l_hand["actuator_names"] + gr3_conf.r_hand["actuator_names"]
        ...
        ee_site_names = [gr3_conf.l_arm["ee_site_name"], gr3_conf.r_arm["ee_site_name"]]
        ...
        ee_site_pos_quat = env.query_site_pos_and_quat_B(ee_site_names, [env.body(gr3_conf.base_body)])
