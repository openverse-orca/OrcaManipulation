...
OrcaStudio streams configured cameras over WebSocket. This module provides
live-frame capture, resolution detection, and timestamp-aligned MP4 frame
...
def select_camera_map(camera_map: dict, enabled: str | None = None) -> dict:
    """按 ``head,wrist_r`` 这类别名过滤相机映射；空值或未匹配时返回原映射。"""
    if not enabled:
        return dict(camera_map)
    names = {item.strip() for item in enabled.split(",") if item.strip()}
    env_names = {_CAMERA_ALIASES.get(name, name) for name in names}
    filtered = {
        env_name: value
        for env_name, value in camera_map.items()
        if env_name in env_names
    }
    return filtered or dict(camera_map)


def make_camera_map(entries: dict[str, tuple[str, int]]) -> dict:
    """构造 ``{env_name: (lerobot_key, port)}`` 相机映射。

    端口与环境相机名属于部署/机器人适配，不在本模块写死。
    """
    return dict(entries)


def camera_thread(cam):
    """读取 CameraWrapper 后台线程。OrcaGym 未提供公共 API，集中封装在此。"""
    return getattr(cam, "thread", None)


def stop_camera_stream(cam) -> None:
    """停止 CameraWrapper 推流。OrcaGym 未提供公共 stop API，集中封装在此。"""
    try:
        cam.running = False
    except Exception:
        pass
...
def bring_up_cameras(
    ...
    while time.time() < deadline:
        ...
        if not pending:
            ...
            return cameras
        for name in pending:
            thread = camera_thread(cameras[name])
