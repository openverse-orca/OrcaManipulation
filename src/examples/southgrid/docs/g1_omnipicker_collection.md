...
2. 请按任务在 OrcaLab 的加载布局对话框中选择对应的布局文件：
   - 四色按按钮任务使用 `src/examples/southgrid/g1_omnipicker/g1_button.json`
   - 工具整理任务使用 `src/examples/southgrid/g1_omnipicker/g1_tool.json`
3. 请确认 G1 OmniPicker 与场景物体已正确加载。
4. 请确认 `src/examples/southgrid/configs/example.yaml` 中的 `level_name` 与 OrcaLab 里的场景名称一致（默认均为 `"example"`）。
...
cd src/examples/southgrid/g1_omnipicker
```
...
请先在 OrcaLab 中加载 `src/examples/southgrid/g1_omnipicker/g1_button.json`，再在上一节的工作目录中运行：
...
任务配置文件位于 `src/examples/southgrid/configs/example.yaml`，其中与采集相关的字段如下：
...
| 文件 | 说明 |
|------|------|
| `src/examples/southgrid/configs/example.yaml` | 采集、回放和推理共用的任务配置，`level_name: "example"` |
| `src/examples/southgrid/g1_omnipicker/g1_tool.json` | OrcaLab 工具整理场景布局 |
| `src/examples/southgrid/g1_omnipicker/g1_button.json` | OrcaLab 四色按按钮场景布局 |
| `src/examples/southgrid/g1_omnipicker/my_waypoint_tool1.yaml` … `my_waypoint_tool5.yaml` | 工具整理采集路点 |
| `src/examples/southgrid/g1_omnipicker/my_slot_waypoints.yaml` | 工具整理辅助路点 |
| `src/examples/southgrid/g1_omnipicker/pose_g1_button_candidates.yaml` | 四色按钮脚本化采集候选位姿 |
...
- 已在 OrcaLab 中加载对应场景布局（`src/examples/southgrid/g1_omnipicker/g1_tool.json` 或 `src/examples/southgrid/g1_omnipicker/g1_button.json`）。
- `src/examples/southgrid/configs/example.yaml` 中的 `level_name` 与场景名称一致。
