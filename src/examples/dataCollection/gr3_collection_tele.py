...
from conf import gr3_conf
from yaml import load, Loader
from dataStorage.gr3_data_storage import Gr3DataStorage
...
def main():
    ...
    for joint_name, value in zip(gr3_conf.r_arm["joint_names"], gr3_conf.r_arm["neutral_joint_values"]):
        default_joint_values[joint_name] = value
    ...
    with open(os.path.join(base_dir, "studio_shop.yaml"), "r", encoding="utf-8") as f:
    ...
    data_storage = Gr3DataStorage(dataset_path=os.path.join(base_dir, "dataset"), hdf5_path="record/proprio_stats.hdf5")
    ...
    data_collection_manager.set_disable_actuator_group([gr3_conf.positions_group])

    orca_logger.info("Creating left hand controller")
    controllers.add_gripper_2f85_reverse_pico_controller(data_collection_manager, env, gr3_conf.l_hand, gr3_conf.base_body, pico_joystick_device, [PicoJoystickKey.X, PicoJoystickKey.Y, PicoJoystickKey.L_TRIGGER])
    
    orca_logger.info("Creating right hand controller")
    controllers.add_gripper_2f85_reverse_pico_controller(data_collection_manager, env, gr3_conf.r_hand, gr3_conf.base_body, pico_joystick_device, [PicoJoystickKey.A, PicoJoystickKey.B, PicoJoystickKey.R_TRIGGER])
    
    orca_logger.info("Creating left arm controller")
    controllers.add_arm_osc_pico_controller(data_collection_manager, env, gr3_conf.l_arm, gr3_conf.base_body, pico_joystick_device, PicoJoystickKey.L_TRANSFORM)
