...
_TV_IMG_HW = (480, 640)  # TeleVuer 占位分辨率（无数采相机）

# 夹爪力帽：F≈τ/r。r=60mm（XML 近端 36mm + 远端 24mm）。
# 目标指尖夹持力 ≤30N → τ_lim = 1.8 N·m（资产默认常为 ±10）。
# 指垫 geom friction slide=2.0（spin/roll=0.1）；满行程静摩擦 Ff≈μF≈60N。
_GRIP_LEVER_M = 0.060
_GRIP_MU_PAD = 2.0
_GRIP_F_MAX_N = 30.0
...
orca_logger = get_orca_logger(
    ...
    log_file="g1_pick_with_gripper_teleop.log",
...
class LoopLagProfiler:
    """主控制环分阶段耗时：定位卡顿来自 ctrl / step / render。

    run_episode 顺序：run_controllers → env.step → env.render。
    每步在下一次 ctrl 开始时结算上一步 total。
    """
    ...
    def install(self, manager, env) -> None:
    ...
    def uninstall(self, manager, env) -> None:
    ...
    def log_summary(self, tag: str = "end") -> None:
        ...
        parts = [
            ...
            for k in ("ctrl", "step", "render", "total")
        ]
        msg = (
...
class TeleopConfig:
    ...
    def from_args(cls, args) -> "TeleopConfig":
        ...
        return cls(
            ...
            diag_gripper=args.diag_gripper,
            diag_gripper_hz=args.diag_gripper_hz,
...
class DiagLog:
    ...
    def __init__(self, cfg: TeleopConfig) -> None:
        ...
        # 诊断日志文件（时间戳）
        _diag_enabled = bool(
            cfg.diag_tele or cfg.diag_joints or cfg.diag_health or cfg.diag_gripper
        )
...
class ControllerWiring:
    ...
    def _setup_tv_events(self, cfg, manager, xr_device, diag):
        ...
        def _on_discard():
            orca_logger.info("[Squeeze] 右 squeeze → 重置本轮")
            print("[SESSION] END reason=right_squeeze_reset", flush=True)
            discard_event.set()
            manager._shutdown_requested = True  # noqa: SLF001

        def _on_shutdown():
            orca_logger.info("[Squeeze] 左右同按 → 退出遥操作")
            print("[SESSION] END reason=both_squeeze_shutdown", flush=True)
            manager._shutdown_requested = True  # noqa: SLF001
        ...
        def _on_disconnect(sustained: bool):
            ...
            if sustained:
                ...
                if tsc is not None and tsc.current_status == TaskStatus.RUNNING:
                    ...
                    print(
                        f"[SESSION] transition {prev.name}→END reason=disconnect_sustained",
                        flush=True,
                    )
...
class HealthMonitor:
    ...
    def start(self) -> None:
        ...
        if cfg.diag_gripper:
            self._threads.append(
                threading.Thread(target=self._gripper_monitor, daemon=True)
            )
        ...
        if cfg.diag_gripper:
            _g_hz = max(0.5, float(cfg.diag_gripper_hz))
            self._diag.log(
                f"[GRIP] monitor ON hz={_g_hz:.1f} "
                f"τ=actuator_force(Nm) Fn=contact normal(N)"
            )
    ...
    # ── Gripper force / torque monitor ─────────────────────────────────────
    def _gripper_monitor(self):
        """读夹爪 pctrl 执行器力矩 + 指尖接触法向力。

        τ：MuJoCo actuator_force（铰链即 N·m），夹持时会被 forcerange 截断。
        Fn：该侧夹爪 geom 与非自身物体的接触法向力之和（N）。
        """
        import mujoco
        ...
        mj = getattr(gym, "_mjModel", None) if gym is not None else None
        md = getattr(gym, "_mjData", None) if gym is not None else None
        if mj is None or md is None:
            diag.log("[GRIP] env.gym._mjModel/_mjData unavailable，夹爪力日志关闭")
            return

        sides = (
            ("L", "gripper_l", g1_omnipicker_conf.gripper_l["actuator_names"]),
            ("R", "gripper_r", g1_omnipicker_conf.gripper_r["actuator_names"]),
        )
        resolved: list[tuple[str, str, list[int], list[str]]] = []
        for label, key, shorts in sides:
            aids: list[int] = []
            tags: list[str] = []
            for short in shorts:
                try:
                    full = env.actuator(short)
                    aid = int(env.model.actuator_name2id(full))
                except Exception:
                    continue
                if aid < 0:
                    continue
                aids.append(aid)
                tags.append(
                    "in" if "inner" in short else ("out" if "outer" in short else short)
                )
            resolved.append((label, key, aids, tags))

        if not any(aids for _, _, aids, _ in resolved):
            diag.log("[GRIP] 未解析到夹爪执行器，夹爪力日志关闭")
            return

        _cforce = np.zeros(6, dtype=np.float64)

        def _contact_fn(side_key: str) -> tuple[float, int]:
            fn_sum = 0.0
            ncon_hit = 0
            ncon = int(md.ncon)
            for i in range(ncon):
                con = md.contact[i]
                n1 = mujoco.mj_id2name(mj, mujoco.mjtObj.mjOBJ_GEOM, int(con.geom1)) or ""
                n2 = mujoco.mj_id2name(mj, mujoco.mjtObj.mjOBJ_GEOM, int(con.geom2)) or ""
                hit1 = side_key in n1
                hit2 = side_key in n2
                if hit1 == hit2:
                    continue
                mujoco.mj_contactForce(mj, md, i, _cforce)
                fn_sum += abs(float(_cforce[0]))
                ncon_hit += 1
            return fn_sum, ncon_hit

        while not self._stop.wait(_g_dt):
            try:
                parts: list[str] = []
                lim_s = "?"
                for label, key, aids, tags in resolved:
                    if not aids:
                        parts.append(f"{label}=n/a")
                        continue
                    taus = [float(md.actuator_force[aid]) for aid in aids]
                    cmds = [float(env.ctrl[aid]) for aid in aids]
                    fhi = max(abs(float(mj.actuator_forcerange[aid, 1])) for aid in aids)
                    flo = max(abs(float(mj.actuator_forcerange[aid, 0])) for aid in aids)
                    lim = max(fhi, flo, 1e-9)
                    lim_s = f"±{lim:.1f}"
                    sat = 100.0 * max(abs(t) for t in taus) / lim
                    fn_sum, ncon_hit = _contact_fn(key)
                    tau_s = " ".join(
                        f"{tag}={tau:+.2f}" for tag, tau in zip(tags, taus)
                    )
                    cmd_s = ",".join(f"{c:+.2f}" for c in cmds)
                    f_est = max(abs(t) for t in taus) / max(_GRIP_LEVER_M, 1e-9)
                    ff_est = _GRIP_MU_PAD * f_est
                    parts.append(
                        f"{label} τ[{tau_s}]Nm sat={sat:.0f}% "
                        f"F≈{f_est:.0f}N Ff≈{ff_est:.0f}N(μ={_GRIP_MU_PAD:.0f}) "
                        f"Fn={fn_sum:.2f}N ncon={ncon_hit} cmd=[{cmd_s}]"
                    )
                _g_n[0] += 1
                diag.log(f"[GRIP] #{_g_n[0]} " + " | ".join(parts) + f" lim={lim_s}")
            except Exception as e:
                _fail_n[0] += 1
                if _fail_n[0] < 3 or _fail_n[0] % 50 == 0:
...
class SessionRunner:
    ...
    def run(self) -> None:
        ...
        while not manager._shutdown_requested:  # noqa: SLF001
            _sess_idx += 1
            orca_logger.info(
                f"[L_ARM] ---- session_prep #{_sess_idx} "
            ...
            if not cfg.local_xml:
                ...
                if not manager.update_scene():
                    orca_logger.info("update_scene 失败，停止遥操作")
                    break
                orca_logger.info(
            ...
            orca_logger.info(f"========== 第 {_sess_idx} 轮 ==========")
            orca_logger.info(f"左臂侧平举锁定 L_INIT={_L_INIT_JOINT_VALUES}")
            if cfg.xr_backend == "pico":
                print(f"\n>>> 第 {_sess_idx} 轮（左Grip开始跟手，再按结束本轮）", flush=True)
            else:
                print(
                    f"\n>>> 第 {_sess_idx} 轮（左squeeze开始跟手，再按结束本轮）",
                    flush=True,
                )
            ...
            if discard_event.is_set():
                ...
                orca_logger.info(f"[SESSION {_sess_idx}] 已重置本轮")
                print(f"[SESSION] reset cleared sess={_sess_idx}", flush=True)
                continue

            if manager._shutdown_requested:  # noqa: SLF001
                print(f"[SESSION] shutdown abort sess={_sess_idx}", flush=True)
                break

            orca_logger.info(f"[SESSION {_sess_idx}] {_dur:.1f}s")
            print(
                f"[SESSION] end sess={_sess_idx} dur={_dur:.1f}s "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="仿真中机器人 actor 名（关节前缀）。uni_test.json 请用 g1_pick_with_gripper_usda_1",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help="TeleVuer 分层健康心跳 [HEALTH]（默认关。也可用环境变量 G1_DIAG_HEALTH=1）",
    )
    parser.add_argument(
        "--diag_joints",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="后台轮询双臂 14 关节 qpos/ctrl/角速度/跟踪误差（默认关；--diag_joints 开）",
    )
    parser.add_argument(
        "--diag_joints_hz",
        type=float,
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="后台轮询夹爪执行器力矩 + 接触法向力（默认开；--no-diag_gripper 关）",
    )
    parser.add_argument(
        "--diag_gripper_hz",
        type=float,
        default=2.0,
        help="夹爪力/力矩诊断采样频率 Hz（默认 2）",
    )
    parser.add_argument(
        "--lag_warn_ms",
        type=float,
    ...
    # ── VR 设备 ──────────────────────────────────────────────────────────────
    print("=" * 60, flush=True)
    print("  g1_pick+OmniPicker 夹爪 纯遥操作启动中...", flush=True)
    print(f"  场景: {cfg.level}", flush=True)
    ...
    if not cfg.local_xml:
        ...
        async def _patched_load_model_xml():
            ...
            # ── 3. 夹爪 pctrl 力矩上限：F_max=30N / r=60mm → ±1.8 N·m ────
            import re as _re

            _grip_fr_n = 0
            _fr_attr = f'forcerange="{_GRIP_FR_XML}"'
            ...
            def _soft_grip_forcerange(m) -> str:
                ...
                if "forcerange=" not in tag:
                    _grip_fr_n += 1
                    return tag[:-2] + f' {_fr_attr}/>' if tag.endswith("/>") else (
                        tag[:-1] + f' {_fr_attr}>'
                    )
                return tag
            ...
            if _grip_fr_n > 0:
                patches_applied.append(
                    f"gripper_forcerange=±{_GRIP_TAU_LIM:g}(n={_grip_fr_n},F≤{_GRIP_F_MAX_N:.0f}N)"
                )
        ...
        orca_logger.info(
            ...
            f" gravcomp={_gc} + gripper_forcerange=±{_GRIP_TAU_LIM:g} "
            f"(F≤{_GRIP_F_MAX_N:.0f}N) + runtime BASE-PIN"
        )
    ...
    try:
        ...
        if scene_ok:
            ...
            # 在 TeleVuer 初始化期间提前做一次哑解算，使后续正式跟手时 solve≤30ms。
            ...
            if wiring.dual_arm_ctrl is not None:
                try:
                    ...
                    orca_logger.warning(f"[IK 预热] 失败（不影响遥操作）: {_we}")

            try:
                gc.collect()
                gc.freeze()
                gc.disable()
                orca_logger.info("[GC] freeze+disable：主循环禁用自动 GC")
            except Exception as _gce:
                orca_logger.warning(f"[GC] 托管失败（不影响遥操作）: {_gce}")
    ...
    if cfg.diag_gripper:
        print(
            f"  夹爪力诊断 [GRIP] @ {cfg.diag_gripper_hz:.0f}Hz "
            f"（τ  F≤{_GRIP_F_MAX_N:.0f}N  Ff=μF μ={_GRIP_MU_PAD:.0f}）→ {diag.log_path}",
            flush=True,
        )
    print("=" * 60, flush=True)
