#!/usr/bin/env bash
# G1 + OmniPicker 夹爪 VR 遥操作数采示例脚本
# 运行前请确认：
#   1. OrcaLab 已启动且已加载 uni_test2.json 场景
#   2. Pico 已通过 USB 连接并已授权 adb
#   3. 已完成 adb 端口转发（见下方 ADB 步骤）

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

# ── 参数配置 ──────────────────────────────────────────────────────────────
OUTPUT_DIR="${1:-/tmp/g1_gripper_collect}"   # 数据集输出目录，可通过第1个参数覆盖
REPO_ID="local/g1_pick_gripper"
TASK="抓取测试"

# ── ADB 步骤（Pico → PC 端口转发 + 打开网页）─────────────────────────────
echo "[ADB] 转发 Pico 8012 端口..."
adb reverse tcp:8012 tcp:8012

echo "[ADB] 在 Pico 端打开 TeleVuer 页面..."
adb shell am start -a android.intent.action.VIEW -d 'http://127.0.0.1:8012/'

# ── 启动采集脚本 ───────────────────────────────────────────────────────────
echo "[采集] 启动 gripper 遥操作采集..."
python -u g1_pick_collection_tele_lerobot_gripper_test.py \
    --level default \
    --task_config ../../common/example.yaml \
    --scene_json uni_test2.json \
    --agent_name g1_pick_with_gripper_usda_1 \
    --task "${TASK}" \
    --lerobot_out "${OUTPUT_DIR}" \
    --repo_id "${REPO_ID}" \
    --fps 20 \
    --clock wall \
    --cameras head,wrist_r \
    --cam_resolution 480x640 \
    --orcagym_addr localhost:50051 \
    --xr_backend televuer \
    --tv_no_tls \
    --tv_goal_mode rebased_tv \
    --tv_ee_dx 0.03 \
    --log_file /tmp/gripper_collect.txt
