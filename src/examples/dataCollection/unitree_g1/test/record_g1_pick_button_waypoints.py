...
一次运行只录一个颜色（``--color red|green|yellow|blue``），默认 3 个 waypoint。
输出目录 ``pose_g1_pick_button/``：只写对应 ``{color}.yaml``（28 维 q）。
...
python -u record_g1_pick_button_waypoints.py \\
  ...
  --color red \\
  --per_color 3 \\
  --output_dir ../pose_g1_pick_button
```

换色：把 ``--color red`` 改成 ``green`` / ``yellow`` / ``blue``，各跑一遍。
...
# unitree_g1/ 父目录需加入 sys.path，以便 import g1_pick_collection_tele_lerobot
# 和 g1_pick_button_waypoints_io（均位于 unitree_g1/ 而非 test/）。
_unitree_g1_dir = os.path.abspath(os.path.join(base_dir, ".."))
if _unitree_g1_dir not in sys.path:
    sys.path.insert(0, _unitree_g1_dir)
...
from g1_pick_button_waypoints_io import (  # noqa: E402
    COLOR_CN as _COLOR_CN,
    COLOR_ORDER as _COLOR_ORDER,
    COLOR_TASK as _COLOR_TASK,
    STATE_DIM,
    write_color_yaml,
)


def _next_prompt(color: str, waypoints: list, per_color: int) -> str | None:
    n = len(waypoints)
    if n < per_color:
        return (
            f"下一步：{_COLOR_CN[color]}按钮 waypoint {n + 1}/{per_color}  "
            f"（已录 {n}/{per_color}）"
        )
    return None
...
def main() -> None:
    ...
    parser.add_argument(
        "--color",
        required=True,
        choices=_COLOR_ORDER,
        help="本次只录该颜色，只写出 {color}.yaml（red/green/yellow/blue）",
    )
    parser.add_argument(
        "--output_dir",
        default="../pose_g1_pick_button",
        help="输出目录（相对脚本目录或绝对路径；本次只写 --color 对应文件）",
    )
    parser.add_argument(
        "--per_color",
        type=int,
        default=3,
        help="本色引导 waypoint 数（默认 3）",
    )
    parser.add_argument(
        "--approach_back",
        type=float,
        default=0.12,
        help="写入 YAML 的 approach_back（米，默认 0.12）",
    )
    ...
    output_dir = (
        args.output_dir if os.path.isabs(args.output_dir)
    ...
    # ── XR 设备（与数采相同构造）──────────────────────────────────────────
    print("=" * 60, flush=True)
    print("  宇树 g1_pick 单色按钮接触关节角录制", flush=True)
    ...
    print(f"  输出: {output_dir}/{color}.yaml", flush=True)
    print(
        f"  颜色: {_COLOR_CN[color]}  引导: {args.per_color} 点  "
        f"task={_COLOR_TASK[color]}",
        flush=True,
    )
    print("=" * 60, flush=True)
    ...
    # ── 场景 / manager（对齐数采）──────────────────────────────────────────
    with open(os.path.join(base_dir, cfg.task_config), "r", encoding="utf-8") as f:
    ...
    print("-" * 60, flush=True)
    print(
        f"  【本次】{_COLOR_CN[color]} × {args.per_color} 点 → "
        f"{color}.yaml（{_COLOR_TASK[color]}）",
        flush=True,
    )
    if cfg.diag_health:
    ...
    def _save_color_yaml(*, quiet: bool) -> None:
        write_color_yaml(
            color,
            waypoints,
            joint_names,
            output_dir,
            approach_back=args.approach_back,
            quiet=quiet,
        )
    ...
    def _handle_record_once() -> None:
        ...
        try:
            ...
            try:
                ...
                pass
            if args.autosave:
                _save_color_yaml(quiet=True)
            nxt = _next_prompt(color, waypoints, args.per_color)
            if nxt:
                print(f"  → {nxt}\n", flush=True)
            else:
                print(
                    f"  → 引导 {args.per_color} 点已满；"
                    f"可继续多录（仍写入 {color}.yaml）或 Ctrl+C 结束\n",
                    flush=True,
                )
        except Exception as e:
    ...
    try:
        ...
        while not _shutdown.is_set() and not manager._shutdown_requested:  # noqa: SLF001
            ...
            hint = _next_prompt(color, waypoints, args.per_color)
            print(
                f"\n>>> 第 {session} 轮 [{_COLOR_CN[color]}] "
                f"（左squeeze开闸；双squeeze记q；"
        ...
        try:
            ...
            orca_logger.error(f"写出 YAML 失败: {e}")
        try:
