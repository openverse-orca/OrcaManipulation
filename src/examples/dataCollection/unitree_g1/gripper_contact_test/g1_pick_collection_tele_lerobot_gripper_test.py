...
_ORCA_MANIP_ROOT = os.path.abspath(
...
_CF_TOOL_KEYS = ("flashlight", "screwdriver", "spanner", "wrench", "knife",
                 "pencil", "hammer", "plier", "tape", "interactive")
_CF_BOX_KEYS = ("toolbox", "tool_box", "_box", "box_", "tray", "crate")
_CF_TABLE_KEYS = ("table", "desk", "workbench", "block", "cabinet", "shelf",
                  "floor", "ground", "plane", "static", "penholder", "wall")
_CF_GRIP_RE = re.compile(r"gripper_(l|r)_(inner|outer)_link(\d+)")
_CF_WRIST_RE = re.compile(r"arm_(l|r)_end_link")
_CF_UUID_RE = re.compile(
    r"_[0-9a-f]{8}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{12}$", re.I
)
_CF_STRIP = ("group_interactive_", "group_static_", "group_", "task_")
_CF_TAIL_RE = re.compile(r"_task_[^_]*_joint$|_geom\d*$|_joint$", re.I)
_CF_AGENT_RE = re.compile(r"^[a-z0-9]+_.*?_usda_\d+_", re.I)
_CF_RANK = {
    "tool": 0, "box": 1, "pad": 2, "grip": 3, "wrist": 4, "table": 5, "robot": 6,
}
# 任一侧命中即记录；夹爪族/桌面之间的组合另行放行
_CF_KEEP_ANY = ("tool", "box")


class ContactForceProbe:
    """工具 / 工具箱 / 桌面 / 夹爪各连杆之间的接触力检测。

    夹爪按 body 细分到 inner_link2、outer_link4、末端腕关节等，
    每 every 步用 mj_contactForce 取法向力与切向力，并从 efc 取
    法向相对速度、切向滑移速度、solver 阻抗，写 CSV 并周期打摘要。

    关键判据：
      sat  = |dist| / solimp宽度  > 1 表示接触刚度饱和（楔入，工具被挤出）
      slip = Ft / (mu * Fn)       > 1 表示已滑动（夹不住）
      vdt  = |vn| * dt            > solimp宽度 表示速度隧穿（穿桌/穿壁）
    """

    def __init__(self, env, out_dir: str, every: int = 10,
                 log_every: int = 50, diag_log=None, tag: str = "g1pick",
                 dt: float = 0.04):
        self.env = env
        ...
        self.path = os.path.join(out_dir, f"contact_{tag}_{ts}.csv")
    ...
    def _mj(self):
        gym = getattr(self.env, "gym", None) or getattr(
            getattr(self.env, "unwrapped", self.env), "gym", None
        )
        if gym is None:
            return None, None
        return getattr(gym, "_mjModel", None), getattr(gym, "_mjData", None)

    @staticmethod
    def _short(name: str) -> str:
        s = _CF_UUID_RE.sub("", name or "")
        s = _CF_AGENT_RE.sub("", s)
        low = s.lower()
        for pre in _CF_STRIP:
            if low.startswith(pre):
                s = s[len(pre):]
                low = s.lower()
        return _CF_TAIL_RE.sub("", s) or "world"

    def _info(self, mj, gid):
        """返回 (cls, part, body_short, geom_short)；part 是细分到连杆的标签。"""
        hit = self._cls_cache.get(gid)
        if hit is not None:
            return hit
        try:
            import mujoco as _mj
            gn = _mj.mj_id2name(mj, _mj.mjtObj.mjOBJ_GEOM, gid) or ""
            bn = _mj.mj_id2name(
                mj, _mj.mjtObj.mjOBJ_BODY, int(mj.geom_bodyid[gid])) or ""
        except Exception:
            gn = bn = ""
        gl, bl = gn.lower(), bn.lower()
        m = _CF_GRIP_RE.search(bl) or _CF_GRIP_RE.search(gl)
        mw = _CF_WRIST_RE.search(bl) or _CF_WRIST_RE.search(gl)
        if "_pad" in gl and "gripper_" in gl:
            cls = "pad"
            part = f"pad_{m.group(1)}{m.group(2)[:2]}{m.group(3)}" if m else "pad"
        elif m:
            cls = "grip"
            part = f"g{m.group(1)}_{m.group(2)[:2]}{m.group(3)}"
        elif mw:
            cls = "wrist"
            part = f"wrist_{mw.group(1)}"
        elif any(k in bl for k in _CF_BOX_KEYS) or any(k in gl for k in _CF_BOX_KEYS):
            cls = "box"
            part = self._short(bn)
        elif any(k in bl for k in _CF_TOOL_KEYS):
            cls = "tool"
            part = self._short(bn)
        elif any(k in bl for k in ("gripper_l_", "gripper_r_", "hand_l_", "hand_r_")):
            cls = "grip"
            part = self._short(bn)
        elif not bl or bl == "world" or any(k in bl for k in _CF_TABLE_KEYS) \
                or any(k in gl for k in _CF_TABLE_KEYS):
            cls = "table"
            part = self._short(gn) if (not bl or bl == "world") else self._short(bn)
        else:
            cls = "robot"
            part = self._short(bn)
        out = (cls, part, self._short(bn), self._short(gn))
        self._cls_cache[gid] = out
        return out

    @staticmethod
    def _keep(c1, c2):
        if c1 in _CF_KEEP_ANY or c2 in _CF_KEEP_ANY:
            return True
        fam = ("pad", "grip", "wrist")
        # 夹爪自碰撞、夹爪撞桌面同样是异常来源，一并留档
        return (c1 in fam and c2 in fam) or \
               (c1 in fam and c2 == "table") or (c2 in fam and c1 == "table")
    ...
    def sample(self):
        ...
        if ncon <= 0:
            return
        step = self.n
        sim_t = float(getattr(md, "time", 0.0))
        buf = np.zeros(6, dtype=np.float64)
        nefc = int(getattr(md, "nefc", 0))
        try:
            efc_vel = np.asarray(md.efc_vel, dtype=np.float64).reshape(-1)
        except Exception:
            efc_vel = None
        try:
            kbip = np.asarray(md.efc_KBIP, dtype=np.float64).reshape(-1, 4)
        except Exception:
            kbip = None
        for i in range(ncon):
            con = md.contact[i]
            ...
            if g1 < 0 or g2 < 0:
                continue
            c1, p1, b1, n1 = self._info(mj, g1)
            c2, p2, b2, n2 = self._info(mj, g2)
            if not self._keep(c1, c2):
                continue
            if _CF_RANK.get(c1, 9) <= _CF_RANK.get(c2, 9):
                cls = f"{c1}|{c2}"
                pa, pb = p1, p2
            else:
                cls = f"{c2}|{c1}"
                pa, pb = p2, p1
            try:
                _mj.mj_contactForce(mj, md, i, buf)
                fn = float(buf[0])
                ft = float(np.linalg.norm(buf[1:3]))
            except Exception:
                fn = ft = 0.0
            dist = float(con.dist)
            try:
                sr = np.asarray(con.solref, dtype=np.float64).reshape(-1)
                srt, srd = float(sr[0]), float(sr[1])
                sw = float(np.asarray(con.solimp, dtype=np.float64).reshape(-1)[2])
                mu = float(np.asarray(con.friction, dtype=np.float64).reshape(-1)[0])
            except Exception:
                srt = srd = sw = mu = 0.0
            sat = abs(dist) / sw if sw > 0 else 0.0
            slip = ft / (mu * fn) if (mu > 0 and fn > 1e-6) else 0.0
            vn = vt = 0.0
            imp = 0.0
            try:
                adr = int(con.efc_address)
                if 0 <= adr < nefc:
                    if efc_vel is not None:
                        vn = float(efc_vel[adr])
                        dim = int(getattr(con, "dim", 3))
                        if dim > 1 and adr + dim <= nefc:
                            vt = float(np.linalg.norm(efc_vel[adr + 1:adr + dim]))
                    if kbip is not None and adr < kbip.shape[0]:
                        imp = float(kbip[adr, 2])
            except Exception:
                pass
            vdt = abs(vn) * self.dt
            tun = vdt / sw if sw > 0 else 0.0
            try:
                cz = float(np.asarray(con.pos, dtype=np.float64).reshape(-1)[2])
            except Exception:
                cz = 0.0
            self._fh.write(
                f"{step},{sim_t:.4f},{cls},{pa},{pb},{b1},{b2},{n1},{n2},"
                f"{dist:.6f},{fn:.3f},{ft:.3f},{mu:.3f},{slip:.3f},"
                f"{srt:.4f},{srd:.3f},{sw:.5f},{sat:.2f},"
                f"{vn:.4f},{vt:.4f},{vdt * 1000:.3f},{tun:.2f},{imp:.4f},{cz:.4f}\n"
            )
            for key, bucket in ((cls, self._agg), (f"{cls} {pa}~{pb}", self._pair_agg)):
                a = bucket.setdefault(key, {
                    "n": 0, "fn_sum": 0.0, "fn_max": 0.0, "pen_min": 0.0,
                    "sat_max": 0.0, "slip_max": 0.0, "tun_max": 0.0, "imp_max": 0.0,
                })
                a["n"] += 1
                a["fn_sum"] += fn
                a["fn_max"] = max(a["fn_max"], fn)
                a["pen_min"] = min(a["pen_min"], dist)
                a["sat_max"] = max(a["sat_max"], sat)
                a["slip_max"] = max(a["slip_max"], slip)
                a["tun_max"] = max(a["tun_max"], tun)
                a["imp_max"] = max(a["imp_max"], imp)

    @staticmethod
    def _fmt(key, a):
        return (
            f"{key}: n={a['n']} Fn均={a['fn_sum'] / max(a['n'], 1):.1f}N "
            f"Fn峰={a['fn_max']:.1f}N 贯穿={a['pen_min'] * 1000:.2f}mm "
            f"饱和={a['sat_max']:.2f} 滑移={a['slip_max']:.2f} "
            f"隧穿={a['tun_max']:.2f} 阻抗={a['imp_max']:.2f}"
        )

    def _summary(self, full: bool = False):
        if not self._agg:
            return f"[CONTACT] #{self.n} 无关注接触"
        order = sorted(
            self._agg.items(),
            key=lambda kv: (-kv[1]["sat_max"], -kv[1]["fn_max"]),
        )
        lines = [self._fmt(k, a) for k, a in order]
        if not full:
            return f"[CONTACT] #{self.n} " + " | ".join(lines[:6])
        pairs = sorted(
            self._pair_agg.items(),
            key=lambda kv: (-kv[1]["sat_max"], -kv[1]["fn_max"]),
        )
        out = [f"[CONTACT][汇总 #{self.n}] 按类别:"]
        out += [f"  {ln}" for ln in lines]
        out.append("  按连杆对（饱和度 top20）:")
        out += [f"    {self._fmt(k, a)}" for k, a in pairs[:20]]
        return "\n".join(out)

    def install(self, env):
        self._orig_render = env.render

        def _render(*a, **k):
            out = self._orig_render(*a, **k)
            self.n += 1
    ...
    def uninstall(self, env):
        if self._orig_render is not None:
            env.render = self._orig_render  # type: ignore[method-assign]
            self._orig_render = None
        self._emit(self._summary(full=True) + f"\n  → {self.path}")
...
def _actor_gids(mj):
    try:
        ...
        for gid in range(int(mj.ngeom)):
            try:
                gn = _mj.mj_id2name(mj, _mj.mjtObj.mjOBJ_GEOM, gid) or ""
                bn = _mj.mj_id2name(mj, _mj.mjtObj.mjOBJ_BODY, int(mj.geom_bodyid[gid])) or ""
            except Exception:
                gn = bn = ""
            if "actormanipulator" in gn.lower() or "actormanipulator" in bn.lower():
                out.append(gid)
        return out
    except Exception:
        return []
...
def _actor_snap(mj, md):
    try:
        import mujoco as _mj
        import numpy as np
        weld_t = int(_mj.mjtEq.mjEQ_WELD)
        done = []
        for i in range(int(mj.neq)):
            if int(mj.eq_type[i]) != weld_t:
                continue
            b1, b2 = int(mj.eq_obj1id[i]), int(mj.eq_obj2id[i])
            n1 = (_mj.mj_id2name(mj, _mj.mjtObj.mjOBJ_BODY, b1) or "").lower()
            n2 = (_mj.mj_id2name(mj, _mj.mjtObj.mjOBJ_BODY, b2) or "").lower()
            if "actormanipulator" not in (n1 + n2):
                continue
            anc = dyn = -1
            for b in (b1, b2):
                if int(mj.body_mocapid[b]) >= 0:
                    anc = b
                else:
                    dyn = b
            if anc < 0 or dyn < 0:
                continue
            mid = int(mj.body_mocapid[anc])
            pos = np.asarray(md.mocap_pos, dtype=np.float64).reshape(-1, 3)[mid]
            quat = np.asarray(md.mocap_quat, dtype=np.float64).reshape(-1, 4)[mid]
            jadr = int(mj.body_jntadr[dyn])
            if jadr < 0 or int(mj.jnt_type[jadr]) != int(_mj.mjtJoint.mjJNT_FREE):
                continue
            qa = int(mj.jnt_qposadr[jadr])
            va = int(mj.jnt_dofadr[jadr])
            md.qpos[qa:qa + 3] = pos
            md.qpos[qa + 3:qa + 7] = quat
            md.qvel[va:va + 6] = 0.0
            if hasattr(md, "qacc_warmstart"):
                md.qacc_warmstart[va:va + 6] = 0.0
            done.append(f"eq[{i}]->anchor")
        return "snap[" + "; ".join(done) + "]" if done else "snap:none"
    except Exception as e:
        return f"snap:err({e})"


def apply_actor_col(env, mode: str) -> str:
    gym = getattr(env, "gym", None) or getattr(getattr(env, "unwrapped", env), "gym", None)
    if gym is None:
        return "actor_col: no gym"
    mj = getattr(gym, "_mjModel", None)
    md = getattr(gym, "_mjData", None)
    if mj is None:
        return "actor_col: no mj"
    mode = str(mode or "keep").strip().lower()
    gids = _actor_gids(mj)
    before = [(int(mj.geom_contype[g]), int(mj.geom_conaffinity[g])) for g in gids]
    snap = ""
    if mode == "off":
        for g in gids:
            mj.geom_contype[g] = 0
            mj.geom_conaffinity[g] = 0
        snap = " " + _actor_snap(mj, md)
    after = [(int(mj.geom_contype[g]), int(mj.geom_conaffinity[g])) for g in gids]
    return f"actor_col={mode} n={len(gids)} gids={gids} {before}->{after}{snap}"
...
class TeleopConfig:
    ...
    def from_args(cls, args) -> "TeleopConfig":
        ...
        return cls(
            ...
            sim_actor_col=str(args.sim_actor_col),
            contact_probe=bool(args.contact_probe),
            contact_every=int(args.contact_every),
        )
...
class ControllerWiring:
    ...
    def wire(
        ...
        _acm = apply_actor_col(env, cfg.sim_actor_col)
        orca_logger.info(f"[PHYS] {_acm}")
        print(f"[PHYS] {_acm}", flush=True)
...
class EpisodeRunner:
    ...
    def run(self) -> None:
        ...
        while not manager._shutdown_requested:  # noqa: SLF001
            ...
            cprobe = None
            if cfg.contact_probe:
                cprobe = ContactForceProbe(
                    env,
                    os.path.join(os.path.expanduser(cfg.diag_log_dir), "contact"),
                    every=cfg.contact_every,
                    log_every=max(cfg.diag_every, 1),
                    diag_log=(self._diag.log if self._diag is not None else None),
                    tag=f"ep{_ep_idx:03d}",
                    dt=float(cfg.time_step) * int(cfg.frame_skip),
                )
                cprobe.install(env)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=30.0,
        help=(
            "内存/NVENC 诊断探针周期（秒，默认 30；设 0 关闭）。"
            "输出帧缓冲字节数、RSS 增速、NVENC 会话归属与每相机编码背压"
        ),
    )
    parser.add_argument(
        "--probe_gpu",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="探针是否查询 GPU NVENC 会话数（默认关；开时优先 pynvml，失败才降频 nvidia-smi）",
    )
    parser.add_argument(
        "--encode_backend",
    ...
    parser.add_argument(
        ...
        choices=("keep", "off"),
        default="off",
        help="off=关闭 ActorManipulator 球碰撞并归位（消除埋地 1000m 的寄生约束，默认 off）",
    )
    parser.add_argument(
        "--contact_probe",
        action="store_true",
        default=False,
        help="开启工具 ↔ 桌面/box/夹爪 接触力检测，写 contact_*.csv 并周期打摘要",
    )
    parser.add_argument(
        "--contact_every",
        type=int,
        default=10,
        help="接触力采样间隔（控制步），默认 10",
    )
    ...
    if not cfg.local_xml:
        ...
        async def _patched_load_model_xml():
            ...
            # ── 3. 夹爪 pctrl 力矩上限临时压到 ±5（资产默认常为 ±10）────────
            import re as _re
            ...
            def _soft_grip_forcerange(m) -> str:
                ...
                if "forcerange=" not in tag:
                    _grip_fr_n += 1
                    return tag[:-2] + ' forcerange="-5 5"/>' if tag.endswith("/>") else (
                        tag[:-1] + ' forcerange="-5 5">'
                    )
                return tag

            xml = _re.sub(r"<general\b[^>]*/?>", _soft_grip_forcerange, xml)
            if _grip_fr_n > 0:
                patches_applied.append(f"gripper_forcerange=±5(n={_grip_fr_n})")
        ...
        orca_logger.info(
            ...
            f" gravcomp={_gc} + gripper_forcerange=±5 + runtime BASE-PIN"
        )
    ...
    if cfg.probe_interval > 0:
        print(
            f"  内存/NVENC 探针 [诊断] @ {cfg.probe_interval:.0f}s 一次",
            flush=True,
        )
    print("=" * 60, flush=True)
    ...
    try:
        ...
        if cfg.probe_interval > 0:
            probe = CollectionProbe(
                writer,
                interval_s=cfg.probe_interval,
                fps_target=cfg.fps,
                probe_gpu=bool(cfg.probe_gpu),
            )
            probe.start()
        with writer:
